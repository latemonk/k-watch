...
var RESILIENCE_INTERVAL_KEY_PREFIX = "resilience:intervals:v9:";
...
var SEED_DOMAINS = {
  ...
  // seed-health alerts at 420min; /api/health uses a wider 720min freshness budget.
  ...
  "comtrade:bilateral-hs4": { key: "seed-meta:comtrade:bilateral-hs4", intervalMin: 17280 },
  // 24d gate in seed-comtrade-bilateral-hs4.mjs
  ...
  "supply_chain:portwatch-ports": { key: "seed-meta:supply_chain:portwatch-ports", intervalMin: 720, minRecordCount: 174 },
...
function currentResilienceCacheFormula() {
  return isEnabledEnv("RESILIENCE_PILLAR_COMBINE_ENABLED", "false") && isEnabledEnv("RESILIENCE_SCHEMA_V2_ENABLED", "true") ? "pc" : "d6";
}
...
async function getSeedBatch(entries) {
  ...
  const activatedMap = /* @__PURE__ */ new Map();
...
async function handler(req) {
  ...
  for (const [domain, cfg] of entries) {
    ...
    if (stale) staleCount++;
    seeds[domain] = {
      status: isError ? "error" : sourceMismatch ? "source_version_mismatch" : probe?.ok === false ? probe.status : coveragePartial ? "coverage_partial" : stale ? "stale" : "ok",
