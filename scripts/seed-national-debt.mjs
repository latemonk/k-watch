...
export function computeEntries(debtPctByCountry, gdpByCountry, deficitPctByCountry, treasuryOverride) {
  ...
  for (const [iso3, debtByYear] of Object.entries(debtPctByCountry)) {
    ...
    // IMF SDMX sometimes returns NGDPD with the unit multiplier already
    // applied (raw USD) instead of billions. No economy exceeds 1e8 billions,
    // so values above that are already USD — don't scale again (2026-07-22:
    // US showed $4.3e22 total debt = 1e9x inflation).
    const gdpUsd = gdpLatest > 1e8 ? gdpLatest : gdpLatest * 1e9;
