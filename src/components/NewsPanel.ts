...
import { escapeHtml, sanitizeUrl, unsafeRawHtml, safeHtml } from '@/utils/sanitize';
...
import { cleanKcgNewsTitle } from '@/config/kcg-news-topics';
import type { KcgNewsTopic } from '@/config/kcg-news-topics';
...
export class NewsPanel extends Panel {
  ...
  // KCG fork: 주제(토픽) 필터 — 카테고리 digest 안에서 소스 부분집합만 표시
  private kcgTopics: KcgNewsTopic[] | null = null;
  private kcgTopicId = 'all';
  private kcgTopicBar: HTMLElement | null = null;
  private kcgAllItems: NewsItem[] | null = null;
  ...
  private createDeviationIndicator(): void {
    // KCG fork: (+289% 같은) 편차 지표는 의미 전달이 안 돼 표시하지 않는다
    // (사장님 피드백 07-21). setDeviation 은 deviationEl null 가드로 무해.
  }
  ...
  private createSortToggle(): void {
    // KCG fork: 정렬 토글(⬇ 아이콘) 제거 — 용도 불명 아이콘 정리(사장님 피드백
    // 07-21). 정렬은 저장된 선호(기본 relevance) 그대로 동작한다.
  ...
  private async handleSummarize(): Promise<void> {
    ...
    // KCG fork: 로딩 표시는 ✨ 버튼 스피너로만 — 인라인 카드가 목록을 밀어내
    // "기사가 사라진" 것처럼 보이던 문제(사장님 피드백 07-21). 결과는 모달.
    ...
    try {
      ...
      if (result?.summary) {
        ...
        showToast(t('components.newsPanel.summaryError'));
      }
    } catch {
      if (!this.element?.isConnected) return;
      showToast(t('components.newsPanel.summaryFailed'));
    } finally {
  ...
  private showSummary(summary: string): void {
    if (!this.element?.isConnected) return;
    // KCG fork: 인라인 카드 대신 모달 — 목록 레이아웃을 건드리지 않는다.
    const title = this.header.querySelector('.panel-title')?.textContent ?? '';
    showKcgModal(`${title} — AI 요약`.trim(), safeHtml`<div style="font-size:13.5px;line-height:1.8">${summary}</div>`);
  }
  ...
  public override showError(message?: string, onRetry?: () => void, autoRetrySeconds?: number): void {
    ...
    // 주제 칩 캐시도 무효화 — 에러/빈 상태 이후 칩 클릭이 옛 목록을 되살리면 안 된다.
    this.kcgAllItems = null;
    super.showError(message, onRetry, autoRetrySeconds);
  }

  // --- KCG fork: 주제 필터 ---

  private get kcgTopicStorageKey(): string {
    return `kcg_topic_${this.panelId}`;
  }

  /** 주제 칩 바를 켠다(politics/intel 등 국내 뉴스 패널 전용). */
  public enableKcgTopicFilter(topics: KcgNewsTopic[]): void {
    this.kcgTopics = topics;
    try {
      const saved = localStorage.getItem(this.kcgTopicStorageKey);
      if (saved && topics.some((topic) => topic.id === saved)) this.kcgTopicId = saved;
    } catch { /* storage unavailable — session-only selection */ }
    this.renderKcgTopicBar();
  }

  private renderKcgTopicBar(): void {
    if (!this.kcgTopics) return;
    if (!this.kcgTopicBar) {
      this.kcgTopicBar = document.createElement('div');
      this.kcgTopicBar.className = 'kcg-topic-bar';
      this.content.before(this.kcgTopicBar);
    }
    this.kcgTopicBar.replaceChildren();
    for (const topic of this.kcgTopics) {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = `kcg-topic-chip${topic.id === this.kcgTopicId ? ' active' : ''}`;
      btn.textContent = topic.label;
      btn.setAttribute('aria-pressed', String(topic.id === this.kcgTopicId));
      btn.addEventListener('click', () => {
        if (this.kcgTopicId === topic.id) return;
        this.kcgTopicId = topic.id;
        try { localStorage.setItem(this.kcgTopicStorageKey, topic.id); } catch { /* ignore */ }
        this.renderKcgTopicBar();
        // 전체 캐시에서 다시 필터해 즉시 재렌더(다음 데이터 갱신을 기다리지 않음)
        if (this.kcgAllItems) this.renderNews(this.kcgAllItems);
      });
      this.kcgTopicBar.appendChild(btn);
    }
  }

  private filterByKcgTopic(items: NewsItem[]): NewsItem[] {
    const topic = this.kcgTopics?.find((candidate) => candidate.id === this.kcgTopicId);
    if (!topic || topic.sources.length === 0) return items;
    const allowed = new Set(topic.sources);
    return items.filter((item) => allowed.has(item.source));
  }

  public renderNews(items: NewsItem[]): void {
    // KCG fork: Google News 쿼리 피드의 "제목 - 매체명" 꼬리 제거(멱등).
    items = items.map((item) => {
      const cleaned = cleanKcgNewsTitle(item.source, item.title);
      return cleaned === item.title ? item : { ...item, title: cleaned };
    });
    if (this.kcgTopics) {
      this.kcgAllItems = items;
      const filtered = this.filterByKcgTopic(items);
      if (filtered.length === 0 && items.length > 0) {
        // 주제에 해당하는 기사만 없는 상태 — 칩 바는 content 밖이라 유지된다.
        this.renderFilteredEmpty('이 주제 기사가 아직 없어요');
        // renderFilteredEmpty 가 방금 캐시를 비웠으니 복원 — 다른 칩으로
        // 갈아타면 전체 캐시에서 다시 필터해 보여줘야 한다.
        this.kcgAllItems = items;
        return;
      }
      items = filtered;
  ...
  public renderFilteredEmpty(message: string): void {
    ...
    this.lastRawClusters = null;
    this.lastRawItems = null;
    // 시간 범위 필터로 빈 상태가 되면 주제 칩 캐시도 버린다(스테일 부활 방지).
    // 주제-빈 상태 경로는 renderNews 쪽에서 캐시를 복원한다.
    this.kcgAllItems = null;
    this.setDataBadge('live');
    this.setCount(0);
    ...
    const html = sorted
      ...
      <div class="item ${item.isAlert ? 'alert' : ''}" ${item.monitorColor ? `style="border-inline-start-color: ${escapeHtml(item.monitorColor)}"` : ''}>
        <div class="item-source">
          <span class="item-source-name">${escapeHtml(item.source)}</span>
          ...
          <span class="item-time-inline">${formatTime(item.pubDate)}</span>
        </div>
