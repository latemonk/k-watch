...
export type MapView = 'global' | 'america' | 'mena' | 'eu' | 'asia' | 'latam' | 'africa' | 'oceania' | 'kr_west' | 'kr_south' | 'kr_jeju' | 'kr_east';
...
export class MapContainer {
  ...
  constructor(container: HTMLElement, initialState: MapContainerState, preferGlobe = false, options: MapContainerOptions = {}) {
    ...
    // KCG fork: 워치리스트 서비스와의 결합을 피하려고 window 이벤트로 받는다.
    // 오버레이 데이터는 kcg-watchlist 엔진이, 포커스는 패널 버튼이 쏜다.
    window.addEventListener('kcg:watch-overlay', this.onWatchOverlay);
    window.addEventListener('kcg:map-focus', this.onWatchFocus);
  }

  private onWatchOverlay = (e: Event): void => {
    const items = (e as CustomEvent).detail;
    if (!Array.isArray(items)) return;
    this.deckGLMap?.setWatchOverlay(items);
  };

  private onWatchFocus = (e: Event): void => {
    const d = (e as CustomEvent).detail as { lat?: number; lon?: number; zoom?: number } | null;
    if (!d || !Number.isFinite(d.lat) || !Number.isFinite(d.lon)) return;
    this.setCenter(d.lat!, d.lon!, Number.isFinite(d.zoom) ? d.zoom : undefined);
  };
  ...
  // KCG fork: DeckGL 유지 + maplibre globe projection 토글(2D/3D 버튼).
  public setGlobeProjection(enabled: boolean): void {
    this.deckGLMap?.setGlobeProjection(enabled);
  }

  public isGlobeProjectionActive(): boolean {
    return this.deckGLMap?.isGlobeProjection() ?? false;
  }
  ...
  public destroy(): void {
    this.destroyed = true;
    this.rendererReady = false;
    window.removeEventListener('kcg:watch-overlay', this.onWatchOverlay);
    window.removeEventListener('kcg:map-focus', this.onWatchFocus);
    this.resizeObserver?.disconnect();
    this.rendererDemandCleanup?.();
    this.rendererDemandCleanup = null;
    this.globeInitToken++;
