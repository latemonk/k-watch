...
export class UnifiedSettings {
  ...
  private render(): void {
    ...
    // KCG fork: 유료(Pro) 표면 제거 — 알림(Upgrade CTA)·API Keys 탭 비노출
    // (사장님 지시 07-21). 업스트림 복원 시 이 상수만 되돌리면 된다.
    const KCG_HIDE_PRO_TABS = true;
    const showNotificationsTab = !this.config.isDesktopApp && !KCG_HIDE_PRO_TABS;
    ...
    setTrustedHtml(this.overlay, trustedHtml(`
      <div class="modal unified-settings-modal">
        ...
        <div class="unified-settings-tabs" role="tablist" aria-label="Settings">
          ...
          ${KCG_HIDE_PRO_TABS ? '' : `<button class="${tabClass('api-keys')}" data-tab="api-keys" role="tab" aria-selected="${this.activeTab === 'api-keys'}" id="us-tab-api-keys" aria-controls="us-tab-panel-api-keys">API Keys <span class="panel-pro-badge">PRO</span></button>`}
        ...
        <div class="unified-settings-tab-panel${this.activeTab === 'settings' ? ' active' : ''}" data-panel-id="settings" id="us-tab-panel-settings" role="tabpanel" aria-labelledby="us-tab-settings">
          ${prefs.html}
          ${KCG_HIDE_PRO_TABS ? '' : this.renderUpgradeSection()}
        </div>
  ...
  private getAvailablePanelCategories(): Array<{ key: string; label: string }> {
    // KCG fork: allowlist 패널이 하나도 없는 카테고리 칩은 숨김(빈 목록 방지).
    const kcgVisible = new Set(KCG_VISIBLE_PANELS);
    return [
      { key: 'all', label: t('header.sourceRegionAll') },
      ...getVariantPanelCategories(this.config.getPanelSettings(), SITE_VARIANT)
        .filter(({ key }) => PANEL_CATEGORY_MAP[key]?.panelKeys.some((panelKey) => kcgVisible.has(panelKey)))
  ...
  private getVisiblePanelEntries(): Array<[string, PanelConfig]> {
    ...
    // KCG fork: 목록 = 실동작·한글 검증 패널 allowlist 만, 한국 관제 목적
    // 우선순위 순서로(사장님 지시 07-21 — 미번역·빈 패널 전부 제거).
    const kcgOrder = new Map(KCG_VISIBLE_PANELS.map((key, i) => [key, i]));
    let entries = Object.entries(panelSettings)
      ...
      .filter(([key]) => kcgOrder.has(key))
      .sort(([a], [b]) => (kcgOrder.get(a) ?? 99) - (kcgOrder.get(b) ?? 99));
