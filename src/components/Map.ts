...
export class MapComponent {
  ...
  public setView(view: MapView, zoom?: number): void {
    ...
    const viewSettings: Record<MapView, { zoom: number; pan: { x: number; y: number } }> = {
      // KCG fork: 'global' (boot/reset) centers the Korean peninsula on the
      // SVG fallback renderer too, mirroring the deck.gl VIEW_PRESETS change.
      global: { zoom: 4, pan: { x: -370, y: 115 } },
      kr_west: { zoom: 6, pan: { x: -363, y: 108 } },
      kr_south: { zoom: 6, pan: { x: -372, y: 122 } },
      kr_jeju: { zoom: 6, pan: { x: -366, y: 128 } },
      kr_east: { zoom: 6, pan: { x: -378, y: 108 } },
