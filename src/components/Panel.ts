...
export class Panel {
  ...
  constructor(options: PanelOptions) {
    ...
    if (options.infoTooltip) {
      ...
      // KCG fork: 툴팁을 패널 내부가 아니라 body 에 포털로 붙인다 — 패널은
      // overflow 클리핑이 있어서 경계에 걸치면 좌우가 잘렸다(07-22 스크린샷).
      // 위치는 열 때마다 (?) 버튼 기준 fixed 좌표로 계산한다.
      ...
      infoBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        // KCG fork: 다른 패널의 열린 툴팁은 닫는다(stopPropagation 때문에
        // document 클릭 핸들러가 못 닫아 동시에 여러 개 떠 있던 버그).
        document.querySelectorAll('.panel-info-tooltip.visible').forEach((el) => {
          if (el !== tooltip) el.classList.remove('visible');
        });
        const visible = tooltip.classList.toggle('visible');
        if (visible) {
          const btnRect = infoBtn.getBoundingClientRect();
          tooltip.style.left = `${Math.round(btnRect.left + btnRect.width / 2)}px`;
          tooltip.style.top = `${Math.round(btnRect.bottom + 8)}px`;
          ...
          requestAnimationFrame(() => {
            ...
            if (rect.bottom > window.innerHeight - margin) {
              tooltip.style.top = `${Math.max(margin, Math.round(btnRect.top - rect.height - 8))}px`;
            }
          });
        }
      });
    ...
    // KCG fork(07-23): 패널 헤더 PRO 배지 제거 — 유료 상품이 없는 포크에서
    // premium 플래그는 업스트림 머지 패리티용 메타데이터로만 남는다.
  ...
  // KCG fork: 카드 확대 — 패널 엘리먼트를 전체 화면 오버레이로 이동해 크게
  // 보여준다(리스너 유지). 닫으면 원래 그리드 위치로 복귀.
  protected appendMaximizeButton(): void {
    const btn = h('button', {
      className: 'icon-btn panel-maximize-btn',
      'aria-label': '크게 보기',
      title: '크게 보기',
    }, '⛶') as HTMLButtonElement;
    btn.addEventListener('click', (e) => {
  ...
  private maximizePlaceholder: HTMLElement | null = null;
  private maximizeOverlay: HTMLElement | null = null;
  private maximizeKeyHandler: ((e: KeyboardEvent) => void) | null = null;

  public toggleMaximize(): void {
    if (this.maximizeOverlay) { this.restoreFromMaximize(); return; }
    const overlay = document.createElement('div');
    overlay.className = 'kcg-panel-overlay';
    ...
    overlay.addEventListener('click', (e) => {
      if (e.target === overlay) this.restoreFromMaximize();
    });
    this.maximizeKeyHandler = (e: KeyboardEvent) => { if (e.key === 'Escape') this.restoreFromMaximize(); };
    document.addEventListener('keydown', this.maximizeKeyHandler);
    this.maximizeOverlay = overlay;
    this.maximizePlaceholder = placeholder;
    this.element.dispatchEvent(new CustomEvent('wm:panel-maximize', { detail: { maximized: true } }));
  }

  private restoreFromMaximize(): void {
    if (!this.maximizeOverlay) return;
    this.element.classList.remove('panel-maximized');
    if (this.maximizePlaceholder?.parentElement) {
      this.maximizePlaceholder.parentElement.insertBefore(this.element, this.maximizePlaceholder);
    } else {
      // Grid was re-rendered while maximized (placeholder detached) —
      // fall back to the panels grid so the card never floats over others.
      document.getElementById('panelsGrid')?.appendChild(this.element);
    }
    this.maximizePlaceholder?.remove();
    this.maximizeOverlay.remove();
    if (this.maximizeKeyHandler) document.removeEventListener('keydown', this.maximizeKeyHandler);
    this.maximizeOverlay = null;
    this.maximizePlaceholder = null;
    this.maximizeKeyHandler = null;
    this.element.dispatchEvent(new CustomEvent('wm:panel-maximize', { detail: { maximized: false } }));
  }
