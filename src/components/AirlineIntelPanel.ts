...
const ALL_TABS = ['ops', 'flights', 'airlines', 'tracking', 'news', 'prices'] as const;
type Tab = typeof ALL_TABS[number];

// KCG fork: 키 없이는 빈 화면인 탭(Flights/Airlines/Prices=aviationstack·
// travelpayouts 키 필요, News=업스트림 RSS 0건 실측)은 숨긴다(사장님 피드백
// 07-21). 코드 경로는 보존 — 키 설정 시 ALL_TABS 로 되돌리면 된다.
const TABS = ['ops', 'tracking'] as const;

const TAB_LABELS: Record<Tab, string> = {
    ops: '공항 현황', flights: 'Flights', airlines: 'Airlines',
    tracking: '실시간 추적', news: 'News', prices: 'Prices',
};
...
export class AirlineIntelPanel extends Panel {
    ...
    constructor() {
        ...
        // KCG fork: ①추적 리스트 클릭 → 지도 하이라이트 ②공항 현황 클릭 → 상세 모달
        this.content.addEventListener('click', (e) => {
            const target = e.target as HTMLElement;
            const trackRow = target.closest('.track-row') as HTMLElement | null;
            if (trackRow?.dataset.icao24) {
                this.content.querySelectorAll('.track-row.kcg-selected').forEach(el => el.classList.remove('kcg-selected'));
                trackRow.classList.add('kcg-selected');
                window.dispatchEvent(new CustomEvent('kcg:highlight-aircraft', {
                    detail: {
                        icao24: trackRow.dataset.icao24,
                        lat: Number(trackRow.dataset.lat),
                        lon: Number(trackRow.dataset.lon),
                    },
                }));
                return;
            }
            const opsRow = target.closest('.ops-row') as HTMLElement | null;
            if (opsRow?.dataset.iata) this.openKcgAirportModal(opsRow.dataset.iata);
        });
    ...
    // ---- Ops tab ----
    // KCG fork: 현재 지도 중심 기준 가까운 공항 순 정렬(사장님 지시 07-21).
    private sortOpsByMapDistance(data: typeof this.opsData): typeof this.opsData {
        const center = (window as unknown as { __kcgMapCenter?: { lat: number; lon: number } }).__kcgMapCenter;
        if (!center) return data;
        const coord = (iata: string) => MONITORED_AIRPORTS.find(a => a.iata === iata);
        const dist = (iata: string): number => {
            const a = coord(iata);
            if (!a) return Number.MAX_SAFE_INTEGER;
            const dLat = a.lat - center.lat;
            const dLon = (a.lon - center.lon) * Math.cos(center.lat * Math.PI / 180);
            return dLat * dLat + dLon * dLon;
        };
        return [...data].sort((x, y) => dist(x.iata) - dist(y.iata));
    }

    private openKcgAirportModal(iata: string): void {
        const s = this.opsData.find(o => o.iata === iata);
        const meta = MONITORED_AIRPORTS.find(a => a.iata === iata);
        const esc = (v: unknown) => escapeHtml(String(v ?? ''));
        const body = document.createElement('div');
        setTrustedHtml(body, trustedHtml(`
          <div style="font-size:13px;line-height:1.9">
            <div><strong>${esc(s?.name || meta?.name || iata)}</strong> <span style="color:#678">(${esc(iata)}${meta?.icao ? ` · ${esc(meta.icao)}` : ''})</span></div>
            ${meta ? `<div><span style="color:#789">위치</span> &nbsp;${esc(meta.city)}, ${esc(meta.country)} &nbsp;·&nbsp; ${meta.lat.toFixed(3)}, ${meta.lon.toFixed(3)}</div>` : ''}
            ${s ? `
            <div><span style="color:#789">운영 상태</span> &nbsp;<strong>${esc(s.severity.toUpperCase())}</strong>${s.closureStatus ? ' · <span style="color:#f43">폐쇄</span>' : ''}</div>
            <div><span style="color:#789">평균 지연</span> &nbsp;${s.avgDelayMinutes > 0 ? `+${s.avgDelayMinutes}분` : '없음'} &nbsp;·&nbsp; <span style="color:#789">결항률</span> ${s.cancellationRate > 0 ? `${s.cancellationRate.toFixed(1)}%` : '0%'}</div>
            ${s.notamFlags.length ? `<div><span style="color:#789">NOTAM</span> &nbsp;${s.notamFlags.map(f => esc(f)).join(', ')}</div>` : ''}
            ${s.totalFlights > 0 ? `<div><span style="color:#789">집계 편수</span> &nbsp;${s.totalFlights.toLocaleString()}편${s.delayPct > 0 ? ` · 지연 ${s.delayPct.toFixed(0)}%` : ''}</div>` : ''}
            ${s.topDelayReasons?.length ? `<div style="margin-top:6px;color:#9fb4c4">주요 지연 사유: ${s.topDelayReasons.slice(0, 3).map(r => esc(r)).join(', ')}</div>` : ''}
            ` : '<div style="color:#789">상세 운영 데이터가 아직 없어요</div>'}
          </div>
        `, 'KCG airport modal — values escaped'));
        showKcgModalNode(`공항 현황 — ${iata}`, body, 460);
    }
    ...
    private renderOps(): void {
        ...
        const rows = this.sortOpsByMapDistance(this.opsData).map(s => `
      <div class="ops-row" data-iata="${escapeHtml(s.iata)}" style="cursor:pointer">
    ...
    private renderTracking(): void {
      ...
      <div class="track-search" style="display:flex;gap:6px;padding:8px 0 6px">
        ...
        <div class="track-row" data-icao24="${escapeHtml(p.icao24)}" data-lat="${p.lat}" data-lon="${p.lon}" style="cursor:pointer">
