...
export type SummarizationProvider = 'ollama' | 'groq' | 'openrouter' | 'browser' | 'cache' | 'generic';
...
// KCG fork: 클라이언트 측 프리미엄 게이트 해제 — 서버 4중 게이트를 다 열어도
// 이 게이트가 익명 사용자를 서버 요약 시도 없이 browser-T5로 떨어뜨려
// AI 인사이트가 계속 '사용 불가'였다. 단일 테넌트·자체 키라 무제한 허용.
configureSummarizeGate(() => true);
void hasPremiumAccess;
...
const API_PROVIDERS: ApiProviderDef[] = [
  // KCG fork: generic = 서버의 LLM_API_URL(Gemini OpenAI 호환) — 이 항목이
  // 없으면 클라이언트가 Gemini를 아예 시도하지 않아 groq/openrouter 키가
  // 없는 자가호스팅에서 AI 인사이트가 영구 '사용 불가'가 된다. featureId는
  // 웹 배포에서 항상 켜져 있는 aiGroq 토글을 재사용(가용성 게이트 통과용).
  { featureId: 'aiGroq',        provider: 'generic',    label: 'AI' },
...
async function runApiChain(
  ...
  for (const [i, provider] of providers.entries()) {
    onProgress?.(stepOffset + i, totalSteps, `${provider.label} 연결 중...`);
...
export async function generateSummary(
  ...
  // KCG fork: callers that don't pin a language get the UI language, so
  // every AI summary/insight surface renders in Korean when locale=ko —
  // English source headlines are translated by the model as part of the
  // summarization prompt (server _shared.ts langInstruction).
  lang = lang || getCurrentLanguage() || 'en';
...
async function generateSummaryInternal(
  ...
  if (BETA_MODE) {
    ...
    if (modelReady) {
      ...
      if (!options?.skipBrowserFallback) {
        onProgress?.(1, totalSteps, '로컬 AI 모델 실행 중(베타)...');
      ...
      if (mlWorker.isAvailable && !options?.skipBrowserFallback) {
        onProgress?.(API_PROVIDERS.length + 1, totalSteps, '로컬 AI 모델 대기 중...');
        const browserResult = await tryBrowserT5(headlines, 'summarization-beta', bodies);
        if (browserResult) return browserResult;
      }

      onProgress?.(totalSteps, totalSteps, '사용할 수 있는 AI가 없어요');
    }
  ...
  if (!options?.skipBrowserFallback) {
    onProgress?.(totalSteps, totalSteps, '로컬 AI 모델 불러오는 중...');
...
export async function translateText(
  ...
  for (const [i, providerDef] of API_PROVIDERS.entries()) {
    ...
    onProgress?.(i + 1, totalSteps, `${providerDef.label} 번역 중...`);
    try {
