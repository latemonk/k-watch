...
export interface PresetDefinitionState {
  panelSettings: Record<string, PanelConfig>;
  panelOrder: string[];
  mapLayers: MapLayers;
}
...
/**
 * Core preset→state builder shared by mission presets and the KCG watch-tab
 * presets: enables exactly the given panels (map always on, dynamic panels
 * preserved) and activates exactly the given layers, sanitized for the variant.
 */
export function applyPresetDefinitionToState(
  preset: { panels: string[]; layers: Array<keyof MapLayers> },
  currentPanelSettings: Record<string, PanelConfig>,
  defaultLayers: MapLayers = DEFAULT_MAP_LAYERS,
  variant: string = SITE_VARIANT,
): PresetDefinitionState {
...
export function applyMissionPresetToState(
  ...
  return {
    preset,
    ...applyPresetDefinitionToState(preset, currentPanelSettings, defaultLayers, variant),
  };
}
