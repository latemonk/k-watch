...
export interface PanelTab {
  ...
  bottomSet: string[];
  /**
   * KCG fork: per-tab map layer snapshot. Tabs created from a watch preset
   * (해양/공중/지상) carry their layer set; switching tabs applies it. Absent
   * on tabs saved before this feature — those leave the live layers alone
   * until their first switch-away refreshes the snapshot.
   */
  mapLayers?: MapLayers;
  /**
   * KCG fork: 이 탭을 만든 감시 프리셋(해양/공중/지상). 프리셋을 따라 표면을
   * 바꾸는 위젯(AI 인사이트 주제 브리프·실시간 속보)이 탭 전환 시 참조한다.
   * 기본 구성 탭·프리셋 도입 전 탭에는 없다.
   */
  kcgPreset?: KcgPresetId;
}
