...
export class DataLoaderManager implements AppModule {
  ...
  async loadNews(): Promise<void> {
    ...
    // KCG fork: no Discord community widget on an ops console.
    ...
    try {
      ...
      // KCG fork: 주제별 브리프(국내/해양·안보) 헤드라인 공급 — 링크 포함(카드 클릭 상세 모달의 근거 기사)
      const toBriefHeads = (items: NewsItem[]) =>
        items.slice(0, 8).map(i => ({ title: i.title, link: i.link, source: i.source }));
      insightsPanel?.setKcgTopicHeadlines?.({
        politics: toBriefHeads(this.ctx.newsByCategory['politics'] ?? []),
        intel: toBriefHeads(this.ctx.newsByCategory['intel'] ?? []),
      });
