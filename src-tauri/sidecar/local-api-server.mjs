...
export async function createLocalApiServer(options = {}) {
  ...
  return {
    ...
    async start() {
      ...
      // KCG all-in-one: the AIS relay lives on loopback inside the same
      // container. Same docker-mode-only trust rationale as the Redis REST
      // proxy above — without this, every maritime snapshot fetch is
      // SSRF-blocked and the vessel layer stays empty.
