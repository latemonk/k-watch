...
export async function isCallerPremium(_request: Request): Promise<boolean> {
  // KCG fork: single-tenant ops console — no Pro product. Handlers that
  // gate data fields on premium status serve every caller. Spend-control
  // identity resolution (resolvePremiumCallerIdentity) is left untouched.
  return true;
}
