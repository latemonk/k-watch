...
export async function getMultiSectorCostShock(
  ...
  if (products.length === 0) {
    // KCG fork: no Railway seed cron — fall back to the on-demand Comtrade
    // public-preview fetch (30-day Redis cache, same key as the seeder).
    const lazy = await lazyFetchBilateralHs4(iso2).catch(() => null);
    if (lazy?.products?.length) products = lazy.products as typeof products;
  }
