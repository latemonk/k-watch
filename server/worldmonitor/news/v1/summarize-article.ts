...
export async function summarizeArticle(
  ...
  const { provider, mode = 'brief', geoContext = '', variant = 'full', lang = 'ko' } = req;
  ...
  // KCG fork: no Pro product — never premium-gate summaries.
  if (false as boolean && requiresPremium && !isPremium) {
    return {
      summary: '',
      model: '',
      provider: provider,
  ...
  const skipReasons: Record<string, string> = {
    ...
    bizrouter: 'BIZROUTER_API_KEY not configured',
      ...
      async () => {
        ...
        const response = await fetch(apiUrl, {
          ...
          body: JSON.stringify({
            ...
            temperature: 0.3,
            // KCG fork: 100 → 2048. Reasoning-capable models (Gemini Flash
            // via the generic OpenAI-compat endpoint) spend hidden thinking
            // tokens from this budget and returned empty content at 100.
            max_tokens: 2048,
            top_p: 0.9,
            ...extraBody,
          }),
