...
// KCG fork: politics/intel 은 클라이언트 주제 칩(정치/경제/사회·해양/북한/재난/지역)이
// 소스 이름 부분집합으로 필터하므로, 중요도 상위 20개 슬라이스만으로는 단일 소스
// 주제가 빈 화면이 된다. 캡을 올리고 소스별 최소 노출(floor)을 보장한다.
const KCG_TOPIC_CATEGORY_CAPS: Record<string, number> = { politics: 60, intel: 60 };
const KCG_TOPIC_SOURCE_FLOOR = 3;
...
async function buildDigest(variant: string, lang: string): Promise<ListFeedDigestResponse> {
  ...
  try {
    ...
    if (variant === 'full') {
      // KCG fork: when the registry defines its own intel feeds (Korean
      // maritime/security set), do NOT append the upstream English
      // INTEL_SOURCES — they would crowd the per-category cap and push the
      // Korean items out entirely.
      if (!feedsByCategory.intel?.length) {
    ...
    for (const [category, items] of results) {
      ...
      const cap = KCG_TOPIC_CATEGORY_CAPS[category] ?? MAX_ITEMS_PER_CATEGORY;
      const sliced = items.slice(0, cap);
      // KCG topic categories: 캡에서 밀려난 소스도 최소 floor 개는 태워
      // 주제 칩이 항상 실제 기사를 보여주게 한다(잔여도 중요도순이라 best-of-rest).
      if (KCG_TOPIC_CATEGORY_CAPS[category]) {
        const perSource = new Map<string, number>();
        for (const item of sliced) perSource.set(item.source, (perSource.get(item.source) ?? 0) + 1);
        for (const item of items.slice(cap)) {
          const count = perSource.get(item.source) ?? 0;
          if (count >= KCG_TOPIC_SOURCE_FLOOR) continue;
          sliced.push(item);
          perSource.set(item.source, count + 1);
        }
      }
      ledgerDrops.perCategoryCap += Math.max(0, items.length - sliced.length);
