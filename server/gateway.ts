...
async function shouldReserveGatewayDirectLlmQuota(request: Request, pathname: string): Promise<boolean> {
  // KCG fork: single-tenant console with its own LLM key — no per-user
  // Clerk-session quota. Without this, anonymous sessions get 401 "Pro
  // authentication required" on summarize → AI 인사이트 영구 '사용 불가'.
